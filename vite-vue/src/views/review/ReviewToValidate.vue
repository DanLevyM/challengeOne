  ...
  setup() {
    ...
    onBeforeMount(async () => {
      try {
        ...
        const res_reviews_logged = await fetch(`${API_URL}/reviews?user_admin_check=/${id_connected[0].id}`);
...
<template>
  <div class="bodyclass">
    <div style="color:white;">
      <div class="rounded" v-for="(critiqueGroup, idFilm) in critiquesGroupedByFilm" :key="idFilm"
        style="margin: 5% 20% 5% 20%; padding-top: 2%; background-color:#111111">
        <h2 style="text-align:center;">Titre du film : {{ critiqueGroup[0].movie }}</h2>
        <div style="margin: 5% 20% 5% 20%; padding-bottom: 2%;">
          <div class="rounded"
            style="color:white; border: 2px solid #1d1b22; background-color: #222028; margin: 2% 2% 4% 2%"
            v-for="critique in critiqueGroup" :key="critique.id">
            <input class="form-check-input" style="margin-left: 2%" v-if="critique.validate == true" type="radio"
              :id="critique.id" :value="critique.description" v-model="picked" checked />
            <input class="form-check-input" style="margin-left: 2%" v-else type="radio" :id="critique.id"
              :value="critique.id" v-model="picked" />
            {{ critique.title }}
            <div>
