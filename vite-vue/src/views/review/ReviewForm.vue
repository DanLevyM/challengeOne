  ...
  <div class="bodyclass">
    <table class="mainTable">
      <thead>
        <tr class="fit">
          <th scope="col">
            <div class="thText">TITRE</div>
          </th>
          <th scope="col">
            <div class="thText">DESCRIPTION</div>
          </th>
          <th scope="col">
            <div class="thText">VALIDATEUR</div>
          </th>
        </tr>
      </thead>
      <tbody class="w-auto">
        <tr v-for="review of reviews_array" :key="review.id" class="fit">
          <td>
            <div class="tableText">{{ review.title }}</div>
          </td>
          <td>
    ...
    <form @submit.prevent="createNewReview(title, description, verif, movieId)">
      ...
      <select v-model="verif">
        ...
        <option v-for="user in administrator" :value="user.id" :key="user.id">{{ user.firstname }} {{ user.lastname }}
        </option>
      </select>
      <span class="col-12 col-sm-6 col-lg-3">
