  ...
  <div>
    <table class="table table-dark col-md-12 col-lg-4 mb-4 mb-lg-4">
      <thead>
        <tr>
          <th scope="col">
            <div class="thText">LIBELLE</div>
          </th>
          <th scope="col">
            <div class="thText">PRIX</div>
          </th>
          <th scope="col">
            <div class="thText">ACTION</div>
          </th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="product in products" :key="product.id">
          ...
          <td>
            <div class="tableText">{{ product.price }}</div>
          </td>
          <td>
            <div class="tableText"><button class="buttonAction delete"><i class="bi bi-trash text-danger"
                  @click.prevent="deleteProduct(product.id)"></i></button>
              <button class="buttonAction edit"><i class="bi bi-pencil-square text-warning"
                  @click="showModals = product.id"></i></button>
            </div>
          </td>
          <modal v-show="showModals == product.id" @close="showModals = false">
            <template v-slot:header>
              <h6 class="modalTitle">Modifier</h6>
            </template>
